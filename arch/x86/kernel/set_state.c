...
ok:
	...
	if(sk_unhashed(sk))
	{
		inet_sk(sk)->inet_sport = htons(desired_port);
		__inet_hash_nolisten(sk, tw);
	}
	spin_unlock(&head->lock);
	if(tw)
	{
...
extern struct vfsmount *sock_mnt __read_mostly;
extern const struct dentry_operations sockfs_dentry_operations;
extern const struct file_operations socket_file_ops;
// set_state version of the sock_alloc file function
// the difference here is that file descriptor fd is not created
// this is done elsewhere in the socket code, to implement file sharing
static struct file* sock_alloc_file(struct socket *sock, int flags)
{
	...
	if (unlikely(!path.dentry)) {
		panic("Could not allocate socket dentry");
	}
...
void restore_tcp_socket(int fd, struct saved_file* f, struct state_info* info)
{
	...
	if(f->flags & O_NONBLOCK)
		flags |=O_NONBLOCK;
	file = sock_alloc_file(sock, flags);
	...
	retval = ip_route_connect(&rt, nexthop, inet->inet_saddr, RT_CONN_FLAGS(sk), sk->sk_bound_dev_if, IPPROTO_TCP,
				  inet->inet_sport, htons(saved_socket->tcp->dport), sk, 1);
	...
	inet->inet_dport = htons(saved_socket->tcp->dport);
	...
	sprint("assigned socket to port %u (%u)\n", inet->inet_sport, ntohs(inet->inet_sport));

	retval = ip_route_newports(&rt, IPPROTO_TCP, inet->inet_sport, inet->inet_dport, sk);
	if(retval)
	{
...
void restore_udp_socket(int fd, struct saved_file* f){
	...
	file = sock_alloc_file(socket, flags);
	fd_install(fd, file);

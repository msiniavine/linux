...
//	lock_sock(sk);
	...
	saved_tcp->daddr = inet->inet_daddr;
	saved_tcp->dport = ntohs(inet->inet_dport);
	saved_tcp->saddr = inet->inet_saddr;
	saved_tcp->sport = ntohs(inet->inet_sport);
	...
	saved_tcp->xmit_size_goal = tp->xmit_size_goal_segs;		
...
static void save_socket_info(struct saved_task_struct* task, struct file* f, struct saved_file* file, struct map_entry* head)
{
	...
	{
	case AF_INET:
			file->socket.inet.daddr = inet->inet_daddr;
			file->socket.inet.rcv_saddr = inet->inet_rcv_saddr;
			file->socket.inet.dport = inet->inet_dport;
			file->socket.inet.saddr = inet->inet_saddr;
			file->socket.inet.num = inet->inet_num;
			file->socket.inet.sport = inet->inet_sport;
